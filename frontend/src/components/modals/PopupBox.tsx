...
export default function PopupBox({type, message, moreinfo, closebt, setSignupProcess, setLoginProcess, normalSubmit} : PopupBoxProps) {
  ...
  const renderMoreInfo = () => {
    if(moreinfo){
      ...
      animate={{opacity: 1}}
      transition={{duration: 1}}
       className={`${!expandInfo && "mt-3"} text-sm inter font-medium text-purple-500 underline flex justify-center items-center cursor-pointer select-none`} onClick={() => setExpandInfo(!expandInfo)}>
      ...
      {expandInfo && (
        ...
        >
          <span className="text-xs inter selection:bg-purple-500 text-slate-500">{moreinfo}</span>
        </motion.div>
      )}
      </>
      )
    ...
    <AnimatePresence>
      {showPopup && (
        ...
        className="flex w-screen h-screen justify-center items-center absolute z-10 bg-[#1a1e15]/45">
          ...
          animate="visible"
          exit="hidden"
           className="flex flex-col relative justify-center items-center z-20 bg-slate-100 rounded-2xl p-2">
            {closebt && (
              <div className="absolute top-0 right-0 mt-2 mr-2">
                <XCircle size={32} weight="fill" className="cursor-pointer text-rose-500 hover:text-rose-500/75" onClick={() => {setShowPopup(!showPopup)
                  if(setSignupProcess){
